...
std::vector<double> NeuralNetwork::activate(std::vector<double> inputs){
    ...
    for(int i = 0; i < layers.size(); i++){
        ...
        for(int j = 0; j < layers[i].neurons.size(); j++){
            ...
            layers[i].neurons[j].output = output[j];
        }
    }
    return outputs;
}
...
double NeuralNetwork::train(std::vector<double> inputs, std::vector<double> targets, double learning_rate){
    ...
    for(int i = layers.size() - 1; i >= 0; i--) {
        ...
        for(int j = 0; j < layers[i].neurons.size(); j++){
            if(i == layers.size() - 1){
                error = target[j] - layers[i].neurons[j].output;
    ...
    for(int j = 0; j < layers[j].neurons.size(); j++){
        for(int k = 0; k < (i == 0 ? inputs.size() : layers[i - 1].neurons.size()); k++){
            if(i == 0){
                layers.[i].neurons[j].weights[k] += learning_rate * layers[i].neurons[j].delta * inputs[k];
            }else{
